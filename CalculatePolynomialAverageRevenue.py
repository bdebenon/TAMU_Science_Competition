...
for x in range(10,78):
    ...
    error = ""
    try:
        df = pd.read_csv('ride_revenues/ride_revenues_community_' + communityNumber + '.csv')
        fits = df.copy()
        days = df['Day'].tolist()
        rides = df['Revenue'].tolist()
        ...
        predictedDf['Revenue_Predicted_Poly'] = predictedAverages[:predictedDf['Date'].count()]
